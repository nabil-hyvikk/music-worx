...
class Search extends Controller
{
    ...
    public function search_tracks(Request $request)
    {
        ...
        if (!$searchString) {
            ...
            $fetch = $this->elastic->search_tracks($searchString, $limit, $offset);
            ...
            if (!empty($fetch['tracks'])) {
               ...
               // $cloud_lib = new Cloud_lib();
                foreach ($result as $key => $value) {
                    ...
                    $value->mp3_preview != $value->mp3_file
                        ? $value->preview_file_url = $this->cloud_lib->get_signed_url($value->mp3_preview, 'music_bucket', 'prev')
                        : $value->preview_file_url = $this->cloud_lib->get_signed_url($value->mp3_sd ?? $value->mp3_file, 'music_bucket', 'new_release');

                    //$value->preview_file_url = $this->getPreviewUrl($value, $cloud_lib);
    ...
    private function getPreviewUrl($value, $cloud_lib)
    {
    ...
    public function getTracksFromIdsRaw($ids)
    {
        ...
        return DB::table('tbl_mp3_mix as t')
            ->select( 't.id', 't.mp3_file', 't.mp3_sd', 't.flac_file', 't.duration', 't.mp3_preview', 't.stream_status', 't.stream_countries', 't.gener', 't.mix_name', 't.song_name', 't.key', 't.bpm', 't.waveform', 't.green_waveform', 't.full_wf', 't.full_gwf', 't.preview_starts', 't.preview_ends', 't.slug', 't.user_id', 't.release_id', 't.id as track_id', 'r.cover', 'r.label', 'r.title', DB::raw('GROUP_CONCAT(u.artist_name SEPARATOR ", ") as artist_names'))
    ...
    public function updateSearchBehaviour($search_keyword, $update_data)
    {
      ...
      public function search_realeses(Request $request)
      {
          ...
          if (!$searchString) {
             ...
             // $elastic = new ElasticSearch();
              $fetch = $this->elastic->search_releases($searchString, $limit, $offset);
    ...
    public function search_artist(Request $request)
    {
       ...
       // $elastic = new ElasticSearch();
        $fetch = $this->elastic->search_artists($searchString, $limit, $offset);
    ...
    public function search_label(Request $request)
    {
       ...
       // $elastic = new ElasticSearch();
        $fetch = $this->elastic->search_labels($searchString, $limit, $offset);
        $result = $fetch['labels'];
